document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!compradorId) {
    document.querySelector(".produtos-lista").innerHTML = "<p>Você precisa estar logado para ver o carrinho.</p>";
    return;
  }
      ...
      produto.innerHTML = `
        ...
        <div class="produto-quantidade">
          <div class="quantidade-controles">
            <button class="btn-quantidade" onclick="diminuirQuantidade(${item.id}, ${item.preco})">-</button>
            <input type="number" value="${item.quantidade}" min="1" class="quantidade-input" id="qtd${item.id}" onchange="atualizarPreco(${item.id}, ${item.preco})">
            <button class="btn-quantidade" onclick="aumentarQuantidade(${item.id}, ${item.preco})">+</button>
          </div>
        </div>
        <div class="produto-preco"><span class="preco-valor" id="preco${item.id}">R$ ${(item.preco * item.quantidade).toFixed(2)}</span></div>
